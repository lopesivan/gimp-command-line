...
# Copyleft :-)
#
# Circular QAM Constelation __2 lines__
...
def gen_constellation_qam_c_2(bg_colour, stars_colour, circles_colour, axis_colour, power):
	...
	i = circles
	while(i > 0):
		draw_stars_circle(img, layer, n/2, i * rad_offset, X_BASE, Y_BASE, i%2 + 1, n)
		i -= 1
	
	float_text = set_text(img, layer, 275, 310, str(n)+"-QAM", 16)
	pdb.gimp_floating_sel_anchor(float_text)
	float_text = set_text(img, layer, 277, 327, "circular-2", 14)
	pdb.gimp_floating_sel_anchor(float_text)
...
register(
	"gen_constellation_qm_c_2",
	"Generate a QAM modulation constelation",
	"Generate a QAM modulation constelation",
	"Juanjo Conti",
	"Juanjo Conti",
	"2006",
	"<Toolbox>/Xtns/Python-Fu/Constellations/circular-QAM-2line",
	"",
	[
